...
import uda_val  # for end-of-epoch mAP
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Directories
    var_save_dir = save_dir / 'weights'  # weights dir
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = uda_val.run(data_dict,
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if (epoch > 0) and (opt.save_period > 0) and (epoch % opt.save_period == 0):
                    # Use str() to convert epoch to string
                    torch.save(ckpt, var_save_dir / f'epoch{str(epoch)}.pt')
                    # torch.save(ckpt, var_save_dir + f'/epoch{epoch}.pt')
                del ckpt
